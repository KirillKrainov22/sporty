...
try:
    ...
    # Данные пользователя (пока тестовые)
    ...
    for achievement_id, achievement in ALL_ACHIEVEMENTS.items():
        if achievement_id not in earned_achievements:
            ...
            # Упрощенная проверка (в реальности нужен парсер)
            upcoming_achievements.append({
                "id": achievement_id,
                **achievement,
                "progress": 0.6  # Пример прогресса
            })
    
    # Показываем прогресс
    for achievement in upcoming_achievements[:3]:  # Только первые 3
